...
mod tests {
    ...
    fn test_subscribe_basic_block_delivery() {
        ...
        runner.start(|mut context| async move {
            ...
            let (total, min) = (peers.len() as u16, (peers.len() / 2) as u16);
            let block = CodedBlock::new(inner, (total, min));

            actor.broadcast(block.clone(), peers).await;

            let subscription_rx = actors[1]
                .subscribe(Some(Round::from((0, 1))), block.commitment())
                .await;
    ...
    fn test_subscribe_multiple_subscriptions() {
        ...
        runner.start(|mut context| async move {
            ...
            let block1 = CodedBlock::new(inner1, (total, min));
            let inner2 = B::new::<Sha256>(block1.digest(), 2, 2);
            let block2 = CodedBlock::new(inner2, (total, min));
