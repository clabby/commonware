...
use crate::{
    threshold_simplex::types::Context, Application, Automaton, Block, Epochable, Supervisor,
};
...
use commonware_cryptography::{
    bls12381::primitives::variant::Variant, Committable, Digestible, Hasher, PublicKey,
};
...
#[derive(Clone)]
pub struct ShardShim<E, A, S, V, B, P, H>
where
    E: Rng + Spawner + Metrics + Clock,
    A: Application<Block = B, Context = Context<H::Digest>>,
    S: Supervisor,
    V: Variant,
    P: PublicKey,
    B: Block<Digest = H::Digest, Commitment = H::Digest>,
    H: Hasher,
{
    ...
    inner: A,
    supervisor: S,
    marshal: super::mailbox::Mailbox<V, B, P, H>,
}

impl<E, A, S, V, B, P, H> Automaton for ShardShim<E, A, S, V, B, P, H>
where
    E: Rng + Spawner + Metrics + Clock,
    A: Application<Block = B, Context = Context<H::Digest>>,
    S: Supervisor,
    ...
    H: Hasher,
{
    ...
    async fn propose(&mut self, context: Self::Context) -> oneshot::Receiver<Self::Digest> {
        ...
        let (parent_view, parent_commitment) = parent;
        let parent_request = if parent_commitment == genesis_digest {
            Either::Left(future::ready(Ok(genesis.inner().clone())))
        } else {
            Either::Right(
                marshal
                    .subscribe(Some(Round::new(0u64, parent_view)), parent_commitment)
                    .await,
            )
        };
        ...
        self.context.with_label("propose").spawn({
            ...
            let participants = self.supervisor.participants(view).unwrap().clone();
            move |context| async move {
                ...
                select! {
                    parent = parent_request => {
                        ...
                        // Erasure encode the block
                        let (total, min) = (participants.len() as u16, (participants.len() / 2) as u16);
                        let block = Block::new(parent_commitment, parent.height() + 1, current);
                        let coded_block = CodedBlock::<_, Sha256>::new(block.clone(), (total, min));

                        let commitment = coded_block.commitment();
                        {
                            let mut built = built.lock().unwrap();
                            *built = Some((view, coded_block));
                        }

                        // Send the commitment to the consensus
                        let result = response.send(commitment);
                        info!(view, ?commitment, success=result.is_ok(), "proposed new block");
                    },
                    _ = response_closed => {
    ...
    async fn verify(&mut self, _: Self::Context, payload: Self::Digest) -> oneshot::Receiver<bool> {
        let (tx, rx) = oneshot::channel();
        // Wait for the shard to be available in a separate task (to continue processing other messages).
        self.context.with_label("verify").spawn({
            let mut marshal = self.marshal.clone();

            // is_participant also works here.
            // let share = self.supervisor.share(view).unwrap().clone();

            move |_| async move {
                // TODO: Need supervisor for share.
                marshal.verify_shard(payload, 0, tx).await;
            }
        });
        rx
    }
...
// impl<E, A, V, B, P, H> Relay for ShardShim<E, A, V, B, P, H>
// where
//     E: Rng + Spawner + Metrics + Clock,
//     A: Automaton<Digest = H::Digest, Context = Context<H::Digest>>,
//     V: Variant,
//     P: PublicKey,
//     B: Block<Digest = H::Digest, Commitment = H::Digest>,
//     H: Hasher,
